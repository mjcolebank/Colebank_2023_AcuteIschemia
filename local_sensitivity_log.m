...
function [sens,nom_sol] = local_sensitivity_log(f,pars,STEP,NORMALIZE,par_ids,logflag)
...
if logflag==1
    [sens] = call_centered_difference_log(pars,f,nom_sol,STEP,NORMALIZE,par_ids);
else
...
function sens = call_centered_difference_log(pars,f,nom_sol,STEP,NORMALIZE,par_ids)
P = par_ids;
Pn = length(P);
outs = size(nom_sol,1);   % Number of QoI
N    = size(nom_sol,2);   % Number of time points
...
if outs==1
    ...
    for i=1:Pn %Note: we only run through the parameters in par_ids
        par_plus  = par0;
        par_plus(P(i)) = par0(P(i)).*(1+STEP); %params + increment
        par_minus  = par0;
        par_minus(P(i)) = par0(P(i)).*(1-STEP); %params + increment
        f_plus = feval(f,par_plus); %Solution of ODE at the increment
        f_minus = feval(f,par_minus); %Solution of ODE at the increment
        sens(:,i) = (f_plus - f_minus)./(2.*STEP); %approximate the derivative
    end
else % for more than one QoI
    sens = zeros(outs,N,Pn);
