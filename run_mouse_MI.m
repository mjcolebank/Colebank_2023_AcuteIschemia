...
% with reduced LV active force generation
% Author: MJ Colebank
...
for mouseID = 1:3
    ...
    num_cycles = 60;                 % Number of cycles to run the model for
    ...
    %% Define the MI Parameters
    % To simulate LV ischemia, the model is run to steady state (e.g., 30
    % heart beats) and then gradually the LV active force generation is
    % decreased, similar to what occurs when coronary arteries are tied off
    NC_induce_MI = 30; % The heart beat when the MI begins
    NC_stop_MI = 40;   % The heart beat when we reach minimal active force
    act_min = 0.25;    % The % reduction in LV active force
    pars.MI = [NC_stop_MI NC_induce_MI act_min];
    ...
    for i=1:num_cycles %Cycle through heart beats
        ...
        ysol = ode15s(@DE_model_mass_MI,tode,IC,options,pars);
        IC = ysol.y(:,end-1);
    ...
    % Get model outputs
    tic
    outputs = get_model_results_MI(toutput,youtput,pars);
    toc
    ...
    %% Plot sarcomere length versus pressure
    figure(40+mouseID);clf
    plot(SL(2,:),p(8,:),'LineWidth',3);
    ylabel('Pressure (mmHg)'); xlabel('Sarc Length (\mum)');
    set(gca,'FontSize',20); grid on;
end
