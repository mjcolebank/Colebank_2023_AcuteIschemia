...
% Wrapper file to call the model and retrun the current residual
function [rfull,model_out] = get_res_GLS(q,pars0,par_ids,data,IC,tspace,rnom,cov)
if ~isstruct(pars0)
    pars = pars0;
    pars(par_ids) = exp(q);
...
if any(temp(:)<0)
    ...
    J = rnom; % Maybe replace with large number here
    return;
end
...
%%
% Dynamic residuals
r_LVP = (p_LV(:)-p_lv_data(:));%./sqrt(n_data_LV);
r_LVV = (V_LV(:)-V_lv_data(:));%./sqrt(n_data_LV);
r_RVP = (p_RV(:)-p_rv_data(:));%./sqrt(n_data_RV);
r_RVV = (V_RV(:)-V_rv_data(:));%./sqrt(n_data_RV);
r_SAP = (p_SA(:)-p_sa_data(:));


rfull = [r_RVP; r_RVV; r_LVP; r_LVV; r_SAP].*sqrt(diag(inv(cov)));
model_out = [p_RV(:); V_RV(:); p_LV(:); V_LV(:); p_SA(:)];
